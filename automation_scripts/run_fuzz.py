...
class Config(object):
    ...
    def get_environ_gcc(self):
        ...
        environ_copy["COPA_ALL_OPTIMIZATION_FLAGS_FILE"] = (
            self.randollvm_root + "/gcc_all_options.txt"
        )
        environ_copy["COPA_CURR_OPTIMIZATION_FLAGS_FILE"] = (
            self.randollvm_root + "/gcc_curr_options.txt"
        )
        return environ_copy

    def set_vars_llvm(self):
        self.fitness_wrapper = self.randollvm_root + "/fitness_wrapper/"
        self.llvm_path = self.randollvm_root + "./build/bin/"
        ...
        self.outputs_folder = (
            self.randollvm_root + "/outputs/" + self.fitness_function + "/"
        )
        self.port = self.server_url.split(":")[-1]

        if self.parallel_mode == 1:
            self.post_processor = (
                self.randollvm_root + "/fitness_wrapper" + "/parallel_postprocessor.so"
            )
        else:
            if str("function_hash") in self.fitness_function:
                self.post_processor = (
                    self.randollvm_root
                    + "/fitness_wrapper"
                    + "/multiarch_llvm_postprocessor.so"
                )
            else:
                self.post_processor = (
                    self.randollvm_root
                    + "/fitness_wrapper"
                    + "/aflpostprocessor_bin.so"
                )
    ...
    def set_vars_gcc(self):
        ...
        self.post_processor = (
            self.randollvm_root + "/fitness_wrapper" + "/generic_postprocessor.so"
        )
...
def fuzz_bitcode_llvm(
    input_file, config, fuzzing_time, instance_time, use_iterations, O3_map
):
    ...
    try:
        subprocess.check_call(
            [
                ...
                input_file,
                "-o",
                config.assembly_folder + "/" + prog_name + ".s",
            ]
        )
    except:
    ...
    try:
        ...
        subprocess.check_call(
            [
                ...
                input_file,
                "-o",
                config.assembly_folder + "/" + str(isSlave) + "/" + prog_name + ".s",
            ]
        )
    except:
        print("llc failed for " + input_file)
...
def compile_O0(input_file, config):
    ...
    try:
        subprocess.check_call(
            [
                config.llvm_path + "/clang",
                clang_arch,
                "-Wl,--unresolved-symbols=ignore-in-object-files",
                "-O0",
                input_file,
                "-o",
                config.original_sources + "/" + prog_name,
            ]
        )
    except:
...
def make_project(input_binary, config, environ):
    ...
    subprocess.call(
        [
            "make",
            "install",
            "DESTDIR=" + config.input_bindirs + input_binary.project_name,
        ],
        env=environ,
        shell=True,
    )
    shutil.copyfile(
        input_binary.bin_path,
        config.downloads + "/original_sources/" + input_binary.bin_name,
    )
    os.chdir(wd)
    end = time.time()
    return end - start
    ...
    while batches_done < num_batches:
        ...
        with pebble.ProcessPool() as executor:
            fuzz = partial(
                fuzz_bitcode_llvm,
                config=config,
                fuzzing_time=config.fuzzing_time,
                instance_time=str(fuzz_time_t),
                use_iterations=False,
                O3_map=O3_compile_time_map,
            )
            try:
        ...
        while True:
            if (time.time() - fuzzing_time) > total_fuzz_time:
                subprocess.call(
                    "killall -9 afl-fuzz clang-12 clang llc llc-12", shell=True
                )
...
def fuzz_bitcode_llvm_parallel(
    isSlave, input_file, config, fuzzing_time, instance_time, use_iterations, O3_map
):
    outputs_folder = config.outputs_folder
    prog_name = input_file.split("/")[-1].replace(".bc", "")
...
def run_llvm_fuzz_parallel(config):
    ...
    with pebble.ProcessPool() as executor:
        compileO3 = partial(
            compile_bc_parallel, input_file=input_files[0], config=config
        )
        try:
...
def run_gcc_fuzz(config):
    if not os.path.isdir(config.input_bindirs):
        print(
            "Please make sure configure_copa.py was run and the gcc_bins folder exists"
        )
...
def main():
    parser = argparse.ArgumentParser(
        description="""HighF-level script to run various
    ...
    parser.add_argument(
        ...
        metavar="fuzzmode",
        type=int,
        help="1 for single source parallel fuzzing, 0 for multi-souce parallel fuzzing",
    )
    ...
    if config.mode == "GCC":
        ...
        if args.m == 0:
            ...
            print(
                "Please choose the correct mode here, see help for the available modes"
            )
            exit(1)
