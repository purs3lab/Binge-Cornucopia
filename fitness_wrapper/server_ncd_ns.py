...
app.config["SQLALCHEMY_DATABASE_URI"] = "postgresql://anon:@localhost/db"
...
def post_file(filename):
    ...
    if request.method == "POST":
        ...
        if (
            AsmFiles.query.filter_by(
                architecture=architectureName,
                binary_name=filename,
                binary_hash=hashNumber,
            ).all()
            == []
        ):
            ...
            ncd_scores = []
            Database = AsmFiles.query.filter_by(
                architecture=architectureName, binary_name=filename
            ).all()
            ...
            if len(Database) > 0:
                for items in Database:
                    ...
                    file_path_current = (
                        DOWNLOADS
                        + "/"
                        + str(architectureName)
                        + "/"
                        + str(filename)
                        + "/"
                        + str(binary_hash_seen)
                    )
                    file_data_current = open(
                        os.path.join(file_path_current), "rb"
                    ).read()
                    current_file_text, arch_name = get_text_sec(
                        file_data_current, filename
                    )
                    ...
                    ncd = (
                        len(combined_compression)
                        - min(
                            len(file_compression_current), len(CurrentFileCompression)
                        )
                    ) / max(len(file_compression_current), len(CurrentFileCompression))
                ...
                print(
                    "The weight that was returned the server is: " + str(return_value)
                )
                ...
                ncd = (
                    len(combined_compression)
                    - min(len(file_compression_O0), len(CurrentFileCompression))
                ) / max(len(file_compression_O0), len(CurrentFileCompression))
                # print(ncd)
                # ncd_scores.append(ncd)
                # if(len(ncd_scores) > 0):
                return_value = ncd
            ...
            if (
                os.path.isdir(
                    DOWNLOADS + "/" + str(architectureName) + "/" + str(filename)
                )
            ) == False:
