...
extern "C" size_t afl_custom_post_process(Config *config, unsigned char *buf, size_t buf_size, unsigned char **out_buf){
    ...
    command += (config->llvm_directory + "/llc -march=aarch64 ");
    ...
    if (compile_status_llc < 0){
        ...
        std::string execption_command(config->llvm_directory + "/llc -march=aarch64 -O0 ");
    ...
    std::string clang_command(config->llvm_directory + "clang --target=aarch64-pc-linux -Wl,--unresolved-symbols=ignore-in-object-files -fstack-size-section -B/evaldisk/akul/BingeSP/BinBench/executable_binutils -gdwarf-3 -mllvm -debug-only=binbench -mllvm -sub -mllvm -split -mllvm -fla -mllvm -bcf " + compiledFile + " -o " + config->assembly_folder + "/binaries/" + config->pname);  
